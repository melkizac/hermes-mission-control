...
type Tab = "org" | "agents" | "queues" | "flows" | "runs" | "outputs" | "permissions" | "health";
type DrawerTab = "overview" | "queue" | "approvals" | "runs" | "outputs" | "activity" | "skills" | "permissions" | "config";
...
function DetailDrawer({ agent, agents, onClose, onAction, onApproveReview, approvingId, actionFeedback }: { agent: OrgAgent; agents: OrgAgent[]; onClose: () => void; onAction: (agent: OrgAgent, action: string, payload?: Record<string, unknown>) => void; onApproveReview: (agent: OrgAgent, review: Review) => void; approvingId?: string | null; actionFeedback?: ActionFeedback }) {
  const [tab, setTab] = useState<DrawerTab>("overview");
  const tabs: DrawerTab[] = ["overview", "queue", "approvals", "runs", "outputs", "activity", "skills", "permissions", "config"];
    ...
    <div className="drawer-backdrop" onClick={onClose}>
      <aside className="org-drawer wide" onClick={(e) => e.stopPropagation()}>
        ...
        {tab === "overview" && <>
          ...
          <section><h3>Operational footprint</h3><div className="org-footprint"><span>{agent.automations.length} automations</span><span>{agent.tasks.length} tasks</span><span>{agent.skills_detail.length || agent.skills.length} skills</span><span>{agent.inbox.length} approvals</span><span>{agent.runs.length} recent runs</span><span>{agent.outputs.length} outputs</span><span>{agent.activity?.length || 0} activity events</span></div></section>
...
export function AgentOrg() {
  ...
  const tabs: Tab[] = ["org", "agents", "queues", "flows", "runs", "outputs", "permissions", "health"];
  ...
  return (
    <div className="agent-org-page scroll">
      ...
      <section className="org-metrics">
        ...
        <Metric label="Cost 7d" value={fmtMoney(data.summary.cost7d ?? 0)} sub={`${data.summary.active_workflows ?? 0} active workflows`} />
      </section>
      ...
      {selected && <DetailDrawer agent={selected} agents={agents} onClose={() => setSelectedId(null)} onAction={handleAction} onApproveReview={handleApproveReview} approvingId={approvingId} actionFeedback={actionFeedback} />}
    </div>
  );
}
