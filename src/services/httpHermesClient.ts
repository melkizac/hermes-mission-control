...
export class HttpHermesClient implements HermesClient {
  ...
  async getAgentRuntimes(): Promise<AgentRuntimeSwitcher> {
    return request<AgentRuntimeSwitcher>("/api/agent-runtimes");
  }

  async saveAgentRuntime(agentId: string, input: AgentRuntimeAssignment): Promise<AgentRuntimeSwitcher> {
    return request<AgentRuntimeSwitcher>(`/api/agent-runtimes/${encodeURIComponent(agentId)}`, {
      method: "POST",
  ...
  async createSpecKitIntake(input: Partial<{ title: string; intent: string; projectId: string; assignee: string; acceptance: string; assumptions: string; priority: number }>): Promise<SpecKitIntakeResponse> {
    return request<SpecKitIntakeResponse>("/api/spec-kit/intake", { method: "POST", body: JSON.stringify(input) });
  }
